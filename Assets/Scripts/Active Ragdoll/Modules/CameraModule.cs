...
namespace ActiveRagdoll {
    ...
    public class CameraModule : Module {
        ...
        protected virtual void Start() {
            // Dynamically find lookPoint from active costume if null or inactive
            if (_lookPoint == null || !_lookPoint.gameObject.activeInHierarchy) {
                _lookPoint = _activeRagdoll.GetPhysicalBone(HumanBodyBones.Head);
            }

            // Check if camera already exists as a child of this GameObject
            Transform existingCamera = transform.Find("Active Ragdoll Camera");
            
            if (existingCamera != null && existingCamera.GetComponent<UnityEngine.Camera>() != null) {
                // Use the existing camera (allows adding post-processing effects in editor)
                Camera = existingCamera.gameObject;
            } else {
        ...
        private void AvoidObstacles() {
            ...
            bool hit = Physics.Raycast(cameraRay, out RaycastHit hitInfo,
                                       Vector3.Distance(Camera.transform.position, _lookPoint.position), ~dontBlockCamera, QueryTriggerInteraction.Ignore);
